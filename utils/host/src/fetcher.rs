                ...
                match (path, request["method"].as_str().unwrap()) {
                    ...
                    ("/node", "optimism_outputAtBlock") => {
                        ...
                        json!({ "result": OutputResponse {
                            ...
                            // [MANTLE] Our own `rpc_types::SyncStatus`, not kona's: pre-Interop
                            // op-node (incl. Mantle production) omits `cross_unsafe_l2` and
                            // `local_safe_l2`, so both are `Option` here. See §3.4c.
                            sync_status: crate::rpc_types::SyncStatus {
